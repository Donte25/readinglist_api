...
from ..utilities import bad_request_if_none

@r.post('/new_reading_list')
@jwt_required()
def handle_create_lists():
    body = request.json
...
def handle_get_lists():
    ...
    response = {
        ...
        'reading_lists': [lists.to_response() for lists in reading_lists]
    }
    return response, 200

@r.get('/<lists_id>')
@jwt_required()
def handle_get_one_lists(lists_id):
    reading_list = ReadingList.query.filter_by(id=lists_id).one_or_none()
...
@r.delete('/lists_delete/<lists_id>')
@jwt_required()
def handle_delete_lists(lists_id):
    reading_list = ReadingList.query.filter_by(id=lists_id).one_or_none()
    if reading_list is None:
        response = {
            'message': 'reading list does not exist'
...
@r.post('/<lists_id>/add_book')
@jwt_required()
def handle_add_book(lists_id):
    body = request.json
...
@r.put('/update/reading_list/<lists_id>')
@jwt_required()
def handle_lists_update(lists_id):
    body = request.json
